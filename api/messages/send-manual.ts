...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const body = (req.body ?? {}) as { patientId?: unknown; message?: unknown };
  ...
  // Mensagem manual é sempre texto livre — não tem template de fallback aqui
  // (isso é só pro disparo automático do formulário). Se a janela de 24h
  // estiver fechada, bloqueia com erro claro em vez de deixar o WhatsApp
  // rejeitar silenciosamente depois.
